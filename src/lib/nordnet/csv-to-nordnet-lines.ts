...
export const toNordnetLines = (csvFiles: CsvFile[]): NordnetLine[] => {
  ...
  const rawNordnetRows = csvFiles.flatMap(({ fileName, data }) => {
    ...
    }).sort((a, b) => a.bokførtDato.getTime() - b.bokførtDato.getTime());
  });
  ...
  const nordnetLines = rawNordnetRows.map<NordnetLine>((row) => {
    ...
    const isInnskudd = hasNordnetType && transaksjonstype === NordnetType.INNSKUDD;

    return {
      referanse: id,
      bokførtDato,
      tilKonto: portefølje,
      inn: isInnskudd ? beløp : 0,
      ut: isInnskudd ? 0 : beløp,
      inngående: isInnskudd ? saldo - beløp : saldo + beløp,
      saldo,
  ...
  return nordnetLines.flatMap((line, index) => {
    ...
    const errors: NordnetError[] = (previousLine !== undefined && previousLine.saldo !== line.inngående) ? [...line.errors, {
      errorType: NordnetErrorType.SALDO_MISMATCH,
      saldo: line.saldo,
      inngående: previousLine.inngående,
    }] : line.errors;
